...
export class Application {
    ...
    private async startUp(iniConf: configInit): Promise<void> {
        ...
        amqpCon.then(async (c) => {
            ...
            c.on("close", (err) => { logger.debug('[AMQP_CLOSE]', err) })
            c.on("error", (err) => { logger.debug('[AMQP_ERROR]', err) })
            ...
            const statsConsumer = new StatsController(amqpService, prepareConsumer, logger, iniConf)
